...
Each native image job runs Trivy 0.66 twice after its component smoke test. The first scan writes a HIGH/CRITICAL JSON artifact with suppressed findings shown and never fails. The second scans only CRITICAL findings, applies the repository's `.trivyignore.yaml`, and exits non-zero for an unaccepted finding. This preserves evidence while giving publication jobs an ordinary blocking dependency.
