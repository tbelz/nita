...
The Ansible, Jenkins, and Robot rows below were refreshed from ARM validation
images on September 15, 2026 after rebasing onto their current upstream
branches. Webapp and Junos MCP retain their August 5 validation results. Counts
are scanner records, so one CVE can appear against multiple binary packages.
Raw JSON reports remain the source of record.

| Image | HIGH | CRITICAL | Accepted CRITICAL CVEs |
| --- | ---: | ---: | --- |
| Ansible | 5 | 0 | None |
| Jenkins | 94 | 2 | CVE-2026-60002, CVE-2026-6653 |
| Robot | 12 | 0 | None |
...
Jenkins retains OpenSSH client and libxml2 findings needed by its lab automation
toolchain; current Debian security updates removed its former Perl exceptions.
Webapp retains the base image's `perl-base` and the MariaDB client library used
by its Python database driver. The full MariaDB command-line client and its
larger Perl dependency tree are not shipped.
